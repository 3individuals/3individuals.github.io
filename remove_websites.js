function remove() {
  ...
  buttons.oncontextmenu = function (button_click) {
    ...
    if (
      button_click.currentTarget != button_click.target &&
      button_click.target.id != "add_button" &&
      button_click.target.id != "box"
    ) {
      if (remove.style.display == "none") {
        ...
        remove.onclick = function () {
          ...
          if (
            buttons_count == 5 ||
            JSON.parse(localStorage.getItem("button")).length == 9
          ) {
